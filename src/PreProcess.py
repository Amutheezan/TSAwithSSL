...
class PreProcess:
    ...
    def pre_process_tweet(self , tweet):
        ...
        tweet = re.sub(r'\\\\' , r'\\' , tweet)
        tweet = re.sub(r'\\x\w{2,2}' , ' ' , tweet)
        tweet = re.sub(r'\\u\w{4,4}' , ' ' , tweet)
        tweet = re.sub(r'\\n' , ' . ' , tweet)
        tweet = re.sub(r'[\w\-][\w\-\.]+@[\w\-][\w\-\.]+[a-zA-Z]{1,4}' , ' ' , tweet)
        ...
        tweet = tweet.replace('\'' , ' ')
        tweet = re.sub(' +' , ' ' , tweet)
        tweet = re.sub(r'([#%&\*\$]{2,})(\w*)' , r'\1\2 ' , tweet)
        tweet = re.sub(r'([^!\?])(\?{2,})(\Z|[^!\?])' , r'\1 \n\3' , tweet)
        tweet = re.sub(r'([^\.])(\.{2,})' , r'\1 \n' , tweet)
        tweet = re.sub(r'([^!\?])(\?|!){2,}(\Z|[^!\?])' , r'\1 \n\3' , tweet)
        tweet = re.sub(r'([^!\?])\?(\Z|[^!\?])' , r'\1 \n\2' , tweet)
        tweet = re.sub(r'([^!\?])!(\Z|[^!\?])' , r'\1 \n\2' , tweet)
        tweet = re.sub(r'([a-zA-Z])\1\1+(\w*)' , r'\1\1\2' , tweet)
        tweet = re.sub(r'([a-zA-Z])\1\1+(\w*)' , r'\1\1\2' , tweet)
        tweet = re.sub(r' [8x;:=]-?(?:\)|\}|\]|>){2,}' , r' ' , tweet)
        tweet = re.sub(r' (?:[;:=]-?[\)\}\]d>])|(?:<3)' , r' ' , tweet)
        tweet = re.sub(r' [x:=]-?(?:\(|\[|\||\\|/|\{|<){2,}' , r' ' , tweet)
        tweet = re.sub(r' [x:=]-?[\(\[\|\\/\{<]' , r' ' , tweet)
        tweet = re.sub(r'(\w+)\.(\w+)' , r'\1\2' , tweet)
